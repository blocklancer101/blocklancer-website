...
def contact(full_name, email, accredited, entity_type, desired_allocation, desired_allocation_currency, citizenship, sending_addr, note, ip_addr):
    ...
    email_types.send_email_type('contact', DEFAULT_SENDER, email)

    sgw.notify_founders(message, subject=full_name + " is interested in the contact")
...
def partners_interest(name, company_name, email, website, note, ip_addr):
    ...
    sgw.notify_admins(message,
                      subject="{name} ({company_name}) is interested in building on Sharing Economy".format(name=name,
