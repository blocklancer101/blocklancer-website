...
@app.route('/contact/join', methods=['POST'])
def join_contact():
    ...
    feedback = mailing_list.contact(full_name, email, accredited, entity_type, desired_allocation, desired_allocation_currency, citizenship, sending_addr, note, request.remote_addr)
    flash(feedback)
    return jsonify("OK")
...
def telegram():
    return redirect(universal.CHATBOT_URL, code=301)
